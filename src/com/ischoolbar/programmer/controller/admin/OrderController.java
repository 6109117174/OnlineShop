...
public class OrderController {
	 ...
	 * list(@RequestParam(name="sn",defaultValue="")String sn,
	 * 
	 ...
	 * Map<String, Object> ret = new HashMap<String, Object>(); Map<String, Object>
	 * queryMap = new HashMap<String, Object>(); queryMap.put("sn", sn);
	 * if(!StringUtils.isEmpty(username)){ Account account =
	 * accountService.findByName(username); if(account != null){
	 * queryMap.put("userId", account.getId()); } } if(moneyMin != null){
	 * queryMap.put("moneyMin", moneyMin); } if(moneyMax != null){
	 * queryMap.put("moneyMax", moneyMax); } if(status != null){
	 * queryMap.put("status", status); } queryMap.put("offset", page.getOffset());
	 * queryMap.put("pageSize", page.getRows()); //ret.put("rows",
	 * orderService.findList(queryMap)); ret.put("total",
	 * orderService.getTotal(queryMap)); return ret; }
	 */
	...
	public Map<String, Object> list(@RequestParam(name="sn",defaultValue="")String sn,
		...
		//queryMap.put("shopname",(String)request.getSession().getAttribute("shopname"));
		orderService.updateShopname();
		ret.put("rows", orderService.findListWithShopname(queryMap));
		//ret.put("rows", orderService.findList(queryMap));
